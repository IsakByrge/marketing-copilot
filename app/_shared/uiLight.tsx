...
// ─────────────────────────────────────────────────────────────
// Ljus spegling av ui.tsx, byggd på themeLight (variant B).
//
// Samma komponent-API, rad för rad — bara paletten skiljer. Finns
// för att stora inline-stilade sidor (Facebook Specialist, Campaign
// Builder, Historik) ska kunna migreras genom att byta importrad i
// stället för att skrivas om.
//
// Nya sidor ska använda primitives.tsx, inte den här. Den här filen
// är en migreringsbrygga och ska tas bort när sista inline-sidan är
// omskriven.
// ─────────────────────────────────────────────────────────────
...
import { T, fontSans, fontSerif, transition } from "./themeLight";
...
export function ResultCard({ children }: { children: ReactNode }) {
  return (
    <div style={{ background: T.surface, border: `1px solid ${T.line}`, borderRadius: 16, padding: "8px 24px", boxShadow: "none" }}>
      {children}
    </div>
  );
}
          ...
          return (
            <div key={i} style={{
              ...
              <span style={{
                ...
                {done ? "✓" : active ? (
                  <span style={{ width: 7, height: 7, borderRadius: "50%", border: `1.5px solid ${T.purpleBorder}`, borderTopColor: T.purpleBright, display: "block", animation: "spin .7s linear infinite" }} />
                ) : null}
              </span>
